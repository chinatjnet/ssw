...
$insert_path = ''.$pic_id.'.jpg';//the origin photo path
...
//insert the photo url into the table
$query="UPDATE `".DB_PRE."self_info` SET `head_pic`='$insert_path' WHERE (`user`='$_SESSION[user]') LIMIT 1";
$db->query($query);

function  log_result($word) {
	@$fp = fopen("log.txt","a");
	@flock($fp, LOCK_EX) ;
	@fwrite($fp,$word."：执行日期：".$new_avatar_path.strftime("%Y%m%d%H%I%S",time())."\r\n");
	@flock($fp, LOCK_UN);
	@fclose($fp);
}
class pic_data
